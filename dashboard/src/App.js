...
const SWARMPIT_API = window.__APP_CONFIG__?.SWARMPIT_URL || process.env.REACT_APP_SWARMPIT_API;
const API_TOKEN = window.__APP_CONFIG__?.SWARMPIT_TOKEN || process.env.REACT_APP_API_TOKEN;
...
function ReplicasMeter({ replicas, min, max }) {
  ...
  // Tacche da 0 (sinistra) fino a rMax (destra)
  const marks = [];
...
function App() {
  ...
  useEffect(() => {
    async function fetchServices() {
      if (!SWARMPIT_API || !API_TOKEN) return;
      try {
        const res = await fetch(`${SWARMPIT_API}/services`, {
        ...
        const autoscaleSvcs = data.filter(s => {
          ...
          return (labels["swarm.autoscale"] || "").toLowerCase() === "true";
        });
  ...
  const fullData = services.map(svc => {
    ...
    return {
      ...
      min: parseInt(labels["swarm.autoscale.min"] || 1, 10),
      max: parseInt(labels["swarm.autoscale.max"] || 10, 10),
              ...
              sortedData.map(row => (
                <tr key={row.id}>
                  ...
                  <td style={{ textAlign: "center" }}>
                    <span className={`status-badge ${row.state === "running" ? "badge-running" : "badge-stopped"}`}>
